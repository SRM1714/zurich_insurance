...
def main():
    ...
    initial_info = np.array(initial_info, dtype=object)  
    ...
    X = np.column_stack((xs, ys, inputs, bdeds, blims, values, num_of_locations))
    y = targets
    ...
    training_idx = len(X_shuffled) - 8000
    ...
    if train_GP:
        ...
        kernel = GPy.kern.RBF(input_dim=5, variance=variance, lengthscale=lengthscale)
    ...
    testing_accounts = accounts + 8000
    ...
    y_test = np.array(targets_test).flatten()
    y_pred = np.array(targets_pred).flatten()
    ...
    exceeding_initial_info = initial_info[training_idx:][exceeding_indices]#[exceeding_indices]
    ...
    exceeding_data.to_csv('definitivo.csv', index=False)
    ...
    exceeding_data['Actual'] = y_test[exceeding_indices]
    exceeding_data['Percentage_Error'] = individual_errors[exceeding_indices]
    exceeding_data.to_csv('todos.csv', index=False)
